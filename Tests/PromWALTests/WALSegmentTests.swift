...
// `GoOracleSupport.RLEHex` is the shared implementation — the head chunk suite needs the same codec, and a
// second copy in another test module is exactly how the two drift apart. Thin aliases keep the call sites
// reading as they did.
func rleHex(_ b: [UInt8]) -> String { RLEHex.encode(b) }
func unrleHex(_ s: String) -> [UInt8] { RLEHex.decode(s) }
