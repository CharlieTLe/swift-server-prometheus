...
**§7f landed the Head itself in six sub-slices** — `isolation.go`, the series index, the `chunkenc` protocol
conformances, `memSeries`' chunk state, `HeadOptions`/`NewHead` and the float append path — and **§7g made it
queryable**. So the port can now INGEST and READ BACK: `head.appender()` → `append` → `commit` puts a sample in
a chunk and a record in the WAL, and `head.index()`/`head.chunks()` find it again, all byte-compared against Go.
**§7h(a) then taught it to forget** — `gc`, `Delete`, `Truncate` and the `MemTombstones` exception 16 had left
out. **§7h(b) then taught it to come back**: `Init` replays the chunk files and the WAL, and the corpus asserts the
equivalence that makes a restart meaningful. **§7h(c) closed §7h and with it `head.go`**: `truncateWAL` and
`wlog/checkpoint.go`, so the WAL no longer grows without bound — a truncation compacts the segments it is about
to delete into a `checkpoint.NNNNNNNN` directory, and `Init` reads that first. What is left of Phase 7 is
`compact.go` and `db.go` — so the port still cannot produce a block outside a test, and the two read-path gaps
§6w records stay open.
...
| 6 — TSDB | **The READ path is closed.** Twenty-three pinned slices. `chunkenc` in full (metadata half, `bstream`, `xor.go`, `xor2.go`, `varbit.go`), the postings algebra and loser tree, `FindIntersectingPostings`, the index READER and WRITER (byte-identical files), `PromFS` (ADR-15), `chunks.Writer`/`Reader`, `meta.json` + `ULID`, `BlockReader`, `PostingsForMatchers`, the label queries with matchers, the deletion-interval arithmetic, `DeletedIterator`, `blockBaseSeriesSet`, both `populateWithDel*` iterators, and both queriers (§6m-§6w). A block Prometheus wrote can be opened, matched, selected, trimmed and read as samples or chunks, every layer pinned against upstream on real files. **The WRITE path is NOT started**: nothing here touches `compact.go`, `head.go`, `db.go` or the WAL, so the port cannot produce a block outside a test. Two read-path gaps stay open by construction and are Phase 7's (`Err` ordering and the undecodable-encoding path both need malformed or non-XOR chunk bytes). Deliberately unported: `ShardedPostings` and `populateChunksFromIterable` — both need the Head. `tombstones`' file reader was the third; it landed in Phase 7 as §7i(t), so exception 16 is now only about the block reader not CALLING it (and §7i(a) renamed that class from `BlockReader` to `Block`, freeing the name for Go's interface) |
| 7 — TSDB write | **TWELVE SLICES LANDED (§7a–§7i(t), §7i(a), §7k and §7j(a)), and the port can now WRITE A BLOCK. §7a: `tsdb/record` in full** — the WAL's wire format, both directions, 469 differential cases. The type table, `MetricType`'s two conversions, and `Encoder`/`Decoder` for Series, Samples V1 **and V2**, Metadata, Tombstones, Exemplars, MmapMarkers and the integer/float histogram records in V1, V2 and custom-buckets flavours. Two new targets mirroring Go's package boundaries: `PromRecord` (`tsdb/record`) and `PromTombstones` (`tsdb/tombstones` — `DeletionIntervals.swift` moved out of `PromBlock`, plus `Stone`). Six quirks recorded, **two of them upstream bugs**: `samplesV2` measures the caller's accumulator to find the record's first entry (quirk 168, which `wlog/checkpoint.go` walks into), and the `minSize` capacity heuristic *discards* that accumulator (exception 18, the one declared divergence — Swift has no `make(len, cap)`). `Decbuf`'s varint reads were quadratic and are not any more. **§7b: `tsdb/wlog`'s segment format** — the 32 KB page framing, the `[type\|flags][BE16 length][BE32 CRC-32C]` fragment header, `WL` with `Log`/`NextSegment`/`Truncate`/`Close`, the segment directory, `SegmentBufReader` and `Reader`'s whole fragment grammar, in a new `PromWAL` target. 43 differential cases driven as a *program* (a segment size, a list of writes, a read range), five of them planting a pre-seeded directory. **§7c: the corruption corpus** — a SECOND input shape whose cases are literal fragments (raw type byte, payload, length/CRC overrides, truncation) rather than a write program, because a write program cannot express "one bit flipped". 37 cases; it took the sweep from **22 survivors to 9** (57 controls, 48 broke) and found a real defect: `NewSegmentsRangeReader`'s two error wraps (`list segment in dir:%v`, `open segment:%v in dir:%v`) were missing. Quirks 179-180, the first being that the faked page padding **erases the torn-record signal**, so a torn final record is normally dropped silently and only a page-aligned cut reports it. **So the port can now write a WAL and read it back, and reject a corrupt one** — uncompressed only (exception 20). Five quirks (174-178), one new declared divergence (exception 19, `Reader.Segment()` returning `-1` where upstream panics), and a `PromFS` POSIX-fidelity fix: a write to a removed path no longer resurrects it. **§7e: `index.MemPostings`** — the Head's in-memory inverted index, deferred from §6d and the first thing `head.go` needs: `add`/`addFor`'s one-pass insert repair, `ensureOrder`, `delete`'s three cleanups, and every reader (`symbols`, `sortedKeys`, `labelNames`, `labelValues`, `all`, `postings`, `postingsForAllLabelValues`, `postingsForLabelMatching`, `iter`). 17 cases, **40 controls scoring 32 broke / 8 survived — and all eight survivors are PROOFS**, argued in the sweep. `Stats` is deferred to Phase 9 with its only caller, the `/status/tsdb` endpoint. Exception 23. **§7d: `tsdb/chunks/head_chunks.go`'s `ChunkDiskMapper`** — the Head's chunk files: the format constants, `ChunkDiskMapperRef`'s arithmetic, `chunkPos`, the writer (`cut`, the CRC discipline, `writeChunk`), the reader (`chunk(ref:)`, `openMMapFiles`, `repairLastChunkFile`), `iterateAllChunks`, `truncate`/`deleteCorrupted`, the out-of-order mask and `GoVarint.uvarintSize` (from `dennwc/varint`, probed against Go over 300k values). 48 differential cases, **61 controls scoring 51 broke / 10 survived**, every survivor argued in the script. Three quirks (181-183) and two exceptions (21-22), and the corpus caught four defects — three of them ADR-15's rather than the format's. **§7f is UNDER WAY and is six sub-slices deep**: (a) `tsdb/isolation.go` in full, (b) `seriesHashmap` + `stripeSeries`, (c) the `Chunk`/`ChunkAppender`/`ChunkIterable` conformances — which also discharged a §6 deferral by teaching `BlockReader` to decode XOR2 — (d) **`memSeries`' in-order chunk state**: the four cut grounds in `appendPreprocessor`, `cutNewHeadChunk`, `append`, `appendable`, `mmapChunks`, `truncateChunksBefore`, the `memChunk` list and `db.go`'s `rangeForTimestamp`, in 58 differential cases with **77 controls scoring 71 broke / 6 survived** and two new quirks (184: `rangeForTimestamp` truncates toward zero; 185: `computeChunkEndTime` relies on Go's wrapping subtraction, which trapped in Swift on the first run). §7f(d) also changed how unexported Go gets verified: the probe is **committed** as `oracle/probe/headmemseries` rather than thrown away, so `verify-fixtures.sh` re-runs it on every pin bump, and (e) **`HeadOptions` and `NewHead`** — the first Head slice with an EXPORTED entry point, so its 25 cases drive `tsdb.NewHead` itself: both validation errors, all four option normalisations, the uninitialised sentinels, `getOrCreate`'s ID allocation, `mmapHeadChunks` and `Close`. 60 controls, **41 broke on the first run and 54 after the corpus was widened** — a constructor-only corpus measures a constructor, not a type. Three quirks (186-188) and exception 24. and (f) **`headAppender`'s float path** — `storage.Appender`, both appenders, `Append`, `Commit`, `commitFloats`, `log()` and `Rollback`, in 40 cases driven through the real `tsdb.Head` and asserted against the accessors, the **WAL records** and the **chunk files** at once; 74 controls, 65 broke. **So the Head ingests**: a sample appended through the port lands in a chunk and in the WAL, byte-compared against Go. Quirks 189-190, the second a defect the corpus found — `Rollback` logs the series records. and (g) **`head_read.go`** — `headIndexReader`, `headChunkReader`, `RangeHead` and the isolation-aware iterator, in 23 cases with 63 controls (51 broke); quirks 191-192, both found by the corpus refusing to generate (an upstream SEGFAULT on a hand-made OOO ref, and `BlockMaxTime` wrapping at `MaxInt64`). **So the Head is now QUERYABLE**: postings, series, chunk metas and chunks come back, and a reader opened mid-commit sees a truncated iterator. and (h)(a) **the Head's GC** — `stripeSeries.gc`, `Head.gc`, `Delete`, `Truncate` and `MemTombstones` (the half exception 16 left out), in 22 cases with 52 controls (41 broke). and (h)(b) **the REPLAY** — `Init`, `loadMmappedChunks`, `removeCorruptedMmappedChunks` and `loadWAL`, in 18 cases whose contract is stated directly (*a replayed Head equals an appended one*) with 47 controls (32 broke). **So a restart works**: a Head reopened on the same directory recovers its series, its samples and its chunk layout, and recovers them from the WAL alone when the chunk files are destroyed. and (h)(c) **`truncateWAL` and `wlog/checkpoint.go`** — the compacted copy a truncation leaves behind so the segments it deletes are not lost, plus `Init`'s backfill of it; 34 differential cases (16 new) committing the checkpoint directory's NAME, its segment BYTES *and* its records decoded, with 24 hand-written assertions for the error strings and the record types the Head cannot produce, and 79 controls scoring 75 broke / 4 survived. `PromWAL` gained `PromRecord`/`PromChunks`/`PromStorage`/`PromTombstones`, because a checkpoint filters records rather than copying bytes. Quirks 193-194 and exception 25 (no rename in `PromFS`, so the temporary directory is COPIED to its final name — end state identical, crash window not). **So `head.go` is DONE and the WAL no longer grows without bound.** and (i)(t) **the tombstone FILE codec** — `Encode`, `Decode`, `WriteFile`, `ReadTombstones` and the magic/version/CRC framing, in 94 differential cases across TWO arms: a 30-case round trip committing the file's BYTES, and a 64-case CORRUPTION arm modelled on §7c whose input is a file DESCRIPTION (magic, version, body, CRC, trailer, truncation) rather than a write program. 52 controls scoring **48 broke / 4 survived, and all four are proofs**. It found a reachable upstream PANIC — an eight-byte `tombstones` file is `runtime error: slice bounds out of range [1:0]` and `OpenBlock` has no recover (quirk 210) — plus the fact that the CRC skips the version byte, so corrupting it is a version mismatch rather than a checksum failure (quirk 211). Quirks 210-213, exception 29 (`MemTombstones.Iter` sorts, because upstream's Go map means a multi-series tombstone file has NO byte order), and exception 25 gained a second site. **Exception 16 is amended**: the codec is done, what is left is the CALL SITE — `PromBlock.Block` (renamed from `BlockReader` by §7i(a)) still does not read the file and there is no `Block.Delete`. and **§7i(a): `tsdb/blockwriter.go` and `LeveledCompactor`'s WRITE path** — `Write`, `write`, `DefaultBlockPopulator.PopulateBlock`, `AllSortedPostings`, the whole of `blockwriter.go`, and `writeMetaFile` plus the two compaction hints from `block.go`, in a new tier-10 `PromCompact` target. 36 differential cases in which upstream's own `BlockWriter`/`LeveledCompactor.Write` writes a block and the port has to produce the same `index`, chunk-segment and `meta.json` BYTES from the same samples, with the ULID scrubbed to a constant and injected back (exception 27, because upstream reads `crypto/rand.Reader` directly and has no seam — quirk 196); the block is then read back with `tsdb.OpenBlock` on one side and `PromBlock.Block` on the other. **62 controls, 53 broke, 9 survived, all nine argued.** Fifteen quirks (195-209) and three exceptions (26 the tombstones-file seam, 27 the ULID source, 28 the copied `.tmp-for-creation` directory). The corpus caught one real defect and it was mis-declared rather than unknown: §6t's XOR-only chunk re-encoder, which turns out to run on **every** compaction of a Head rather than only on a deletion, because the open head chunk's index meta says `MaxInt64` and `trimBack` therefore always fires (quirk 208). Two API decisions were taken here because §7j is the next caller: `PromBlock.BlockReader` was renamed to **`PromBlock.Block`** so `BlockReader` can be Go's interface, and the `Compactor` protocol is declared **in full** with `plan`/`compact` throwing a named error. **So the port can produce a block outside a test.** And **§7j(a): `storage/merge.go` in full**, plus `lazy.go`, `secondary.go` and `generic.go` Part A — the VERTICAL merge, ROADMAP's oldest deferral, landed ahead of the `db.go` slice that needs it. 96 differential cases through `NewMergeQuerier`/`NewMergeChunkQuerier` over REAL block queriers, with 138 controls scoring 114 broke / 24 survived. It also gave `PromBlock` its `Querier`/`SeriesSet` CONFORMANCES (§6v/§6w had left the block query path as free functions over narrowed protocols), which is the protocol shape §7j and Phase 9 build on, and it unblocks §7i(a)'s `CompactError.verticalCompactionUnported`. Exceptions 30 and 33, quirks 215-219. and **§7k: `chunkenc`'s two HISTOGRAM encodings** — `histogram.go` and `float_histogram.go` in full, plus the encoding half of `histogram_meta.go` (the layout header, `putZeroThreshold`, `putCustomBound`, `expandSpansBothWays`, `insert`, `adjustForInserts`) that Phase 5 left behind when it took only the decision half. 288 differential cases across two corpora generated from ONE shape list, so the places where the two encodings genuinely disagree — a schema change reads back as `UnknownCounterReset` from the integer chunk and `NotCounterReset` from the float one (quirks 220-221) — are a diff between two fixture files rather than a fact buried in two unrelated corpora. 108 controls scoring **96 broke / 12 survived**, five of the twelve deliberately inert. Ten quirks (220-229) and two exceptions (31-32, the iterators' slice recycling and `Compact`'s no-op). The corpus found the harness first — recording a Go slice from a REUSED `AtHistogram` buffer made 83 of 130 cases "fail" against a port whose only sin was value semantics — and then four real gaps, closed by a harvested witness (`1.001`, whose `*1000` lands below an integer) and three span layouts nobody would write by hand. `newEmptyChunk` answers for all four encodings and `FromData` landed with it. It also amends §7i(a): that slice's note that the histogram encodings would "fall out for free" from the re-encoder was half right — the chunk construction did, the sample LOOP did not, and both of upstream's histogram arms had to be ported into `populateCurrForSingleChunk` (quirk 208's amendment). The Head's histogram APPEND path is the slice §7k unblocks and is still deferred. **Still not started: `db.go`.** One of §6w's two read-path gaps is closed (the undecodable-encoding path) and one is not (`Err` ordering — see §7i(a) for the route in). **The next slice is §7j (`db.go`)** |
...
Green as of this commit: **340,434 committed fixture lines, 732 tests** across 25 test targets, on both
...
`tsdb/record` **53 of 55**, `memSeries`' chunk state **71 of 77**, the Head's core **54 of 60**, the float
append path **65 of 74**, the read path **51 of 63**, the GC and deletion path **41 of 52**, the WAL replay
**32 of 47**, the tombstone file codec **48 of 52**, the block writer **53 of 62**, the histogram
chunk encodings **96 of 108**, the vertical merge **114 of 138**, with every
survivor's argument written into the source or the
sweep's tail, next to the code it concerns.
...
**A third shape, and it is not a budget failure at all — it is a coercion failure.** An untyped integer
*expression* does not convert inside an ARRAY LITERAL on 6.1: `ranges: [2 * 60 * 60 * 1000]` against an
`[Int64]` parameter is `cannot convert value of type 'Int' to expected element type 'Int64'` there and
compiles silently on 6.4. §7i(a) shipped three of them and the floor job was the only thing that noticed. The
same rule as the bullets above, one step further: **spell an `Int64` constant as the value it denotes, with
the derivation in a comment**, and give it a name if more than one call site wants it.
...
non-XOR chunk bytes, which no block the port can currently write contains. §7i (`compact.go`) is the slice
that closes them, being the first thing that can put such bytes in a fixture.

**Phase 6 does NOT include the write path.** Nothing here touches `compact.go`, `head.go` or the WAL. The
...
**§6w is Phase 6's last slice.** Phase 7 starts at §7a.
...
**DONE.** All nine steps landed. `Chunk`, `ChunkAppender` and `ChunkIterable` are conformed by `XORChunk`,
`XOR2Chunk` and their appenders and iterators; `BoxedFloatChunkIterator` is one public generic adapter in
`PromChunkEnc` and both hand-written `BoxedXOR` copies are gone; `newEmptyChunk` exists; and
**`BlockReader.samples` now decodes XOR2**, which discharges the §6 deferral that pointed here. `verify-fixtures`
still passes, so widening that read path changed no committed output.

Two behaviours are worth keeping in view because the Head will lean on them:

- **`XORAppender.append(st:t:v:)` discards `st` and that is byte-for-byte faithful** — appending through the
  three-argument protocol method with a wild start timestamp produces the same bytes as the two-argument
  concrete one, and `XOR2Appender` with two different `st` values produces *different* bytes. Both are
  asserted, and together they are quirk 36 made testable.
- **The histogram arms raise rather than trap**, with Go's exact panic strings. Unreachable by contract, since
  `appendPreprocessor` cuts a new chunk on an encoding change — exception 9's treatment.

`newEmptyChunk` answers for the two float encodings and **names** the rest rather than substituting XOR,
because the histogram chunk encodings genuinely are not ported. So a block carrying one is now a loud failure
instead of the silent short read `BlockReader` used to produce. *(Superseded by §7k: both histogram
encodings are ported and `newEmptyChunk` answers for all four. The rest of this section stands.)*
...
The **histogram** chunk encodings were genuinely absent when this was written (`Sources/PromChunkEnc/` had
`XORChunk`, `XOR2Chunk`, `Bstream`, `Varbit` and `HistogramMeta`, but no
`HistogramChunk`/`FloatHistogramChunk`), so `newEmptyChunk` could only answer for the two float encodings.
**§7k closed that**, and it is the slice this paragraph predicted: `chunkOpts.useXOR2` still selects between
exactly the two float encodings, because the Head's histogram APPEND path is a separate slice again.

§7f is COMPLETE for the float path: the `Chunk` conformance prerequisite above, `memSeries`'s chunk state
(§7f(d)), `HeadOptions`/`NewHead` (§7f(e)) and `headAppender` (§7f(f)) are all landed. What remains of the Head
is deferred by feature, not by omission — histograms and exemplars in the appender, `UpdateMetadata`, the
out-of-order arm — plus `Init`, which is §7h's because its body is the replay. **§7g has landed too** (below), so the
next slice is **§7h, `head_wal.go`** — the replay, and with it `Head.Init` and `gc()`.
...
#### §7f(e): `HeadOptions` and `NewHead` — LANDED, and the first slice with an exported entry point

`HeadOptions`/`DefaultHeadOptions`/`UseXOR2FloatEncoding`, `NewHead` with both validation errors and all four
option normalisations, `resetInMemoryState`, the time-window state and its sentinels, `updateMinMaxTime`,
`SetMinValidTime`, `appendableMinValidTime`/`AppendableMinValidTime`, `initialized`, `compactable`,
`OverlapsClosedInterval`, `NumSeries`/`NumStaleSeries`, `Meta`, `String`, `Size`, `getOrCreate` and
`getOrCreateWithOptionalID`, `onChunkCreated`, `mmapHeadChunks`, `Close`, `mmappedChunksDir`, `HeadStats` and
`WALReplayStatus`. **25 differential cases against the real `tsdb.NewHead`; 60 controls scoring 54 broke, 6
survived.** Three quirks (186-188) and one exception (24).

**No probe package, and that is the point.** `NewHead` is exported and so is everything this corpus asks about,
so `oracle/suites_head_core.go` drives upstream's own constructor. It is worth noticing *what* the exported
surface gives you: not just the answers, but **`NewHead`'s mutations of the options object it was handed** —
the negative OOO window it clamps, the `MaxExemplars` it zeroes, the nil `SeriesCallback` it fills, the
non-positive `WALReplayConcurrency` it replaces. All four fields are exported, so the corpus reads them back
after construction. That is also why the port's `HeadOptions` is a **class**: `db.go` keeps the same pointer and
reads those fields later, so a value type would silently lose the writes.

The sweep is the part to learn from. **It scored 41 of 60 on its first run**, and almost every survivor was a
corpus gap rather than a proof: a *fresh* Head answers so little that the stripe size, the postings' order, the
shard hash, both lifecycle callbacks, `Meta`'s series count, `appendableMinValidTime`'s two boundaries and
everything `Close` does were all unobserved. **A constructor-only corpus measures a constructor, not a type.**
Thirteen closed by asserting state the corpus cannot reach (a Head with series and chunks in it), and one by
committing the head DIRECTORY listing rather than only the chunk directory's contents — which is what makes the
name `chunks_head` observable at all.

The six remaining survivors are argued in the sweep's tail: the deliberate harness check, **four tautologies
that together say `mmapReady` is a pure performance shortcut** (every test it makes is redundant with one
elsewhere — the transition test with the increment-by-one, the stripe skip with the per-series check, the
per-series check with `memSeries.mmapChunks`' own guard, and `n > 0` with the fact that a two-chunk series
always writes one), and `Head.closed`, whose only upstream reader is `head_read.go:452` and therefore §7g's.

Three things worth carrying:

- **`Init` is NOT here, deliberately.** Its body *is* the replay — `loadMmappedChunks`, the chunk snapshot, the
  record loop — and its deferred `gc()` needs `stripeSeries.gc`. The one thing it does that is not replay is
  `h.minValidTime.Store(...)`, which is `setMinValidTime`. So an appender can run against a Head whose
  `minValidTime` was set directly, which is what §7f(f) will do; porting a hollow `Init` would have claimed the
  replay works.
- **The uninitialised Head is encoded in the DATA, not in a flag** (quirk 186): `minTime == MaxInt64` *is*
  `initialized() == false`, so `Meta()` reports an inverted range until the first sample. Clamping it to 0
  would break `Truncate`'s early return and `db.go`'s compaction scheduling.
- **`defaultWALReplayConcurrency` is fixed at 1** rather than `GOMAXPROCS(0)` (exception 24), because the
  option travels in the fixture and a host-dependent input is an unreproducible corpus. The *substitution* is
  still pinned — the corpus commits whether `NewHead` replaced the value, not the number.

That slice is §7f(f), below, and it is what made `updateMinMaxTime`, `getOrCreate` and the WAL records
corpus-visible.

#### §7f(f): `headAppender`'s float path — LANDED. **The Head can now ingest.**

`storage.Appender` (deferred since Phase 5 "to the phase that first has a caller"), `initAppender`,
`Head.Appender`/`appender`, `initTime`, `sampleType`, `appendBatch`, `headAppender.Append`,
`AppendSTZeroSample`, the appender's `getOrCreate` with both validations, `getCurrentBatch`'s whole decision
table, `GetRef`, `log()`, `Commit`, `appenderCommitContext`, `commitFloats`, `handleAppendableError`,
`unmarkCreatedSeriesAsPendingCommit` and `Rollback`. Plus `labels.HasDuplicateLabelNames`, which the appender
needs. **40 differential cases through the real `tsdb.Head`; 74 controls scoring 65 broke, 9 survived.** Two
quirks (189, 190).

**A sample appended through this port now lands in three places that are all byte-compared against Go**: the
accessors, the **WAL records**, and the **chunk files** after `Close` m-maps the tail. That is the whole reason
the slice is worth its size — a corpus with only the accessors would pass on a port that never wrote a byte.

**The corpus found a real defect, and it was in `Rollback`.** Upstream's `Rollback` ends with `return a.log()`
*after* truncating the batches, so the samples are dropped but the series records are still written — *"Series
are created in the head memory regardless of rollback. Thus we have to log them to the WAL in any case."* The
port had read the first forty lines of the function and stopped. Two rollback cases were the only mismatches on
the first run, both a missing 28-byte series record. Quirk 190. **Read to the closing brace.**

One divergence worth knowing: **`appendSTZeroSample` returns Go's `(ref, error)` pair rather than throwing**,
because it is the one appender method whose error comes back with a NON-ZERO ref (`ErrOutOfOrderST` carries the
series' own ref so the caller can still append the real sample). Same treatment as `memSeries.appendable`. The
fixture is what forced it: the port threw, and the ref came back 0 where Go had 2.

Four things the sweep had to be taught, and they generalise to any appender-shaped corpus:

- **Two appenders must OVERLAP.** `commitFloats` re-checks `appendable`, and the answer can only differ from
  `Append`'s if another transaction committed in between — the input grew an `interleaved` mode that opens every
  appender before any append.
- **A duplicate label NAME cannot come from a map.** `labels.FromMap` de-duplicates by construction; the corpus
  needs a flat pair list to reach `label name "%s" is not unique`.
- **`headMaxt` is visible only through the OOO window.** It appears in exactly one decision, so distinguishing
  it needs a head whose minTime is far below its maxTime and a sample just outside the real window.
- **The compaction-window floor MOVES** (`max(maxTime - chunkRange/2, minValidTime)`), so a sample that was
  appendable early in a program is out of bounds later. Only a program shows that.

The nine survivors are argued in the sweep's tail, and the largest family is worth stating here: **the second
`appendable` check and the whole `appenderCommitContext` counter set are metrics-only.** When a verdict changes
between Append and Commit, `memSeries.append` rejects the sample anyway on its own `c.maxTime >= t` test, so
the chunks, the WAL and every accessor agree either way — only the counters differ, and `headMetrics` is Phase 9.

Still deferred inside §7f, each with its owner: histograms and exemplars in the appender, `UpdateMetadata`, the
out-of-order commit arm, and `Head.Init` (the replay, §7h).

#### §7g: `head_read.go` — LANDED. **The Head is queryable.**

`headIndexReader` (`Index`, `indexRange`, `Symbols`, `SortedLabelValues`, `LabelValues`, `LabelNames`,
`Postings`, `PostingsForLabelMatching`, `PostingsForAllLabelValues`, `SortedPostings`, `Series`,
`LabelNamesFor`), `appendSeriesChunks`, `memSeries.headChunkID`, `unpackHeadChunkRef`, `RangeHead` in full,
`headChunkReader` (`Chunks`, `chunksRange`, `ChunkOrIterable`, `ChunkOrIterableWithCopy`), `chunkFromSeries`,
`memSeries.chunk`, `memSeries.iterator`, `safeHeadChunk`, `stopIterator` and `makeStopIterator`. **23
differential cases through the real `tsdb.Head`/`tsdb.NewRangeHead`; 63 controls scoring 51 broke, 12
survived.** Two quirks (191, 192), both found by the corpus refusing to generate.

**This is where isolation finally does something visible.** A reader takes an `isolationState` when it is
created, and `memSeries.iterator` turns that into a `stopAfter` — so a chunk with four samples iterates two if
the reader opened between the two commits that wrote them. The corpus expresses it with a `readerAfterPhase`
field, and that one field is what makes §7f(a)'s whole file observable for the first time.

Three findings worth carrying:

- **Upstream SEGFAULTS on an OOO chunk ref for a series that never took an out-of-order sample** (quirk 191).
  `chunkFromSeries` routes bit 23 straight to `oooChunk`, which dereferences a nil `s.ooo`. `promoracle`
  crashed while generating the corpus. The port answers `ErrNotFound` — a deliberate divergence, because a
  hand-made ref is not a crash worth reproducing.
- **`RangeHead.BlockMaxTime` wraps** at `maxt == MaxInt64` (quirk 192), which is what a caller asking for
  "everything" passes. Swift's checked `+` trapped; the operator is `&+`.
- **A reader's window can be EMPTY, and that is a different path from an invisible one.** `chunksRange` clamps
  `mint` up to `MinTime()`, which is `MaxInt64` on an uninitialised head — so a reader opened before the first
  sample rejects every chunk as out of range rather than truncating it. Reaching the `stopAfter == 0` arm needs
  a later commit to cut a NEW chunk.

The sweep started at 34 of 63, and the lesson is the same shape as §7f(e)'s: **a read-path corpus that never
m-maps, never closes the head, never opens a reader early and never asks for a window that excludes a chunk is
exercising one path in four.** `mmapHeadChunks` is unexported and only `db.go` and `Close` call it, so the
m-mapped read path is pinned by hand-written assertions built on facts the corpus does pin — that is the honest
seam, and it is written into the test.

The twelve survivors are argued in the sweep's tail: two log-only guards that need `gc()` (§7h), four
tautologies (including the discovery that the chunk-id bound and the head-chunk lookup answer `ErrNotFound`
for the same inputs, so the bound is not the safety property), five features not yet reached (the head-chunk
cache, a middle-chunk isolation boundary that only replay can build, the `RangeHead` chunk-window path, the
isolation-state close), and the deliberate harness check.

#### §7h(a): the Head FORGETS — `gc`, `Delete`, `Truncate` and `MemTombstones`

`stripeSeries.gc`, `iterForDeletion`, `Head.gc`, `Head.Delete`, `Head.Tombstones`, `Truncate`,
`truncateMemory`, `truncateSeriesAndChunkDiskMapper`, `clampInterval`, and **`MemTombstones`** — the half of
`tombstones.go` exception 16 left out, which had to land here because `Head.Tombstones()` returns it. **22
differential cases through the real `tsdb.Head`; 52 controls scoring 41 broke, 11 survived.**

§7f made the Head ingest, §7g made it queryable, and this makes it forget — the third of the four things
`db.go` asks of it. The fourth is replay, which is §7h(b).

Two closures from the sweep worth carrying, because both are about what a corpus has to COMMIT rather than
about the code:

- **A GC corpus must commit the RAW postings**, not only the series that resolve. `Series` skips a missing
  series by design, so a ref the GC failed to remove reads as "no change". Committing `allPostings` plus a few
  label-indexed postings is what made `postings.delete` and its `affected` argument observable at all.
- **The appendable-window clamp needs a surviving series ABOVE the floor.** `minTime` moves up to the GC's
  `actualMint` but never past `appendableMinValidTime()`, and in a dense corpus those two are equal. Samples at
  0-1000 and 20000-21000 with a truncation at 15000 separate them: 19000 clamped, 20000 not.

**Six of the eleven survivors are the same tautology — a second clamp downstream makes the first redundant** —
and they are argued individually in the sweep's tail. The most useful is `Delete`'s: it clamps the requested
interval to the head's range and then clamps AGAIN per series, and a series' range can never be wider than the
head's, so the first clamp is documentation. `Delete`'s `indexRange(mint, maxt)` is the same kind of thing: the
window only affects `LabelValues`/`LabelNames` and the chunk metas, neither of which `Delete` reads.

The remaining survivors name their owner: `Truncate`'s captured `initialized()` flag has no reader until
`truncateWAL` lands (§7h(b)), a series with mmapped chunks and no head chunk is only buildable by replay, and
`minMmapFile` needs two chunk FILES — 128 MiB each — which only `db.go`'s compaction ages out.

#### §7h(b): the Head REPLAYS — `Init`, `loadMmappedChunks`, `loadWAL`. **A restart works.**

`Head.Init` (the float path: m-mapped chunks, then the WAL segments, then the three deferred actions),
`loadMmappedChunks`, `removeCorruptedMmappedChunks`, `loadWAL` for series/samples/tombstones,
`resetSeriesWithMMappedChunks`, `appendChunkAndMmap` and `replayChunkOpts`. **18 differential cases; 47 controls
scoring 32 broke, 15 survived.**

**The corpus IS the contract.** Each case builds a Head, writes through it, then opens a second Head on the same
directory and calls `Init` — and commits the state of both, so the fixture asserts §7f's promise directly: *a
Head built by replaying a WAL equals a Head built by appending the same samples.* The cases where `equivalent`
is deliberately FALSE are the informative ones: a `minValidTime` above part of the data discards it, and a
`minValidTime` past everything replays a full WAL into an empty head.

Four things worth carrying:

- **The unclean-shutdown cases never close the first head, and that is forced.** Two live `ChunkDiskMapper`s on
  one directory make upstream's own writer panic (*"expected newly cut file to have sequence:offset 1:8, got
  2:8"*), so a case that closes the first head *after* the second opens crashes the oracle. Leaking it is what a
  crash looks like anyway.
- **A corrupt chunk file has to keep its HEADER.** `NewChunkDiskMapper` validates the magic number when it opens
  the directory, so a wholly garbled file fails construction and `removeCorruptedMmappedChunks` never runs.
  Corrupting the body is what reaches the recovery — and the case proves it: twelve samples come back from the
  WAL with the chunk file destroyed.
- **`mmMaxTime`'s skip is a fast path, not a correctness property** — and finding that out closed a loop. Removing
  it changes nothing, because `appendPreprocessor`'s "the timestamp is already in the mmapped chunks" rejection
  refuses those samples anyway. §7f(d) recorded that route as unreachable without replay; replay is what reaches
  it.
- **`Init`'s three deferred actions subsume three of the filters below them.** The `minValidTime` chunk filter,
  the unknown-tombstone guard and more all agree with the deferred clamp + `gc()`, which is why five of the
  survivors are tautologies rather than gaps. They are argued individually in the sweep's tail.

`truncateWAL` and the checkpoint are **§7h(c)**, below.

#### §7h(c): the WAL stops growing — `truncateWAL` and `wlog/checkpoint.go`. **§7h and `head.go` are DONE.**

`Head.truncateWAL`, `walExpiries`/`getWALExpiry`/`updateWALExpiry`, `keepSeriesInWALCheckpointFn`, and the whole
of `tsdb/wlog/checkpoint.go` — `CheckpointStats`, `CheckpointDir`, `listCheckpoints`, `LastCheckpoint`,
`DeleteCheckpoints`, `DeleteTempCheckpoints` and `Checkpoint` itself — plus `Head.Init`'s checkpoint backfill,
which §7h(b) had left as a named gap. **34 differential cases (16 new) and 24 hand-written assertions; 79
controls scoring 75 broke, 4 survived.**

This is what stops the WAL growing without bound: a truncation compacts the segments it is about to delete into
a `checkpoint.NNNNNNNN` directory in the same segmented format, deletes them, and `Init` reads the checkpoint
before the live segments. `PromWAL` gained `PromRecord`/`PromChunks`/`PromStorage`/`PromTombstones` to do it,
because a checkpoint FILTERS records rather than copying bytes.

**The corpus commits the checkpoint as bytes AND as decoded records**, and both were needed. The bytes are the
contract — a dropped series record is a byte diff. The decoded form is what makes a failure legible: `records`
lists `2={__name__="m2", job="b"}` so "the checkpoint kept a series record for a series the head no longer has"
reads directly out of the fixture instead of out of RLE hex. Adding it cost twenty lines and closed the gap
between "the bytes differ" and "here is why".

Five things worth carrying:

- **A checkpoint needs FOUR segments, and the corpus needs five phases to get them.** `last` is decremented
  first (the live segment is never checkpointed) and then `first + (last-first)*2/3` must come out above
  `first` — so three segments give `(2-1)*2/3 == 0` and nothing happens. The threshold itself is asserted by
  hand in `HeadReplayTests`, because no corpus case sits on it.
- **The dropped-SERIES arm needs TWO truncations, and finding that out was the corpus's main discovery.** `gc`
  records `walExpiries[ref] = actualInOrderMint`, which is always `>= mint`, so `keep` answers *true* for every
  series the same truncation collected — the expiry arm keeps it. `keep` can only answer false once the expiry
  has gone stale, and the expiries are pruned *after* the checkpoint, so it takes a second truncation at a
  higher `mint`. `two-checkpoints-drop-a-series-record` is that case, and it doubles as the only case with two
  checkpoints in sequence — which is what exercises `Checkpoint`'s range replacement (`from` becomes the
  previous index + 1 and that directory is prepended to the read).
- **A tombstone can only reach the checkpoint's filter if the delete happens BETWEEN phases.** At the end of a
  case the stone lands in the live segment, which is never checkpointed. Hence the `deleteAfterPhase` knob — and
  hence quirk 194, found while writing the assertion: the record codec flattens a `Stone`'s intervals into one
  entry each, so upstream's multi-interval `break` is dead on anything that came off a WAL.
- **`Init` takes only `endAt` from `Segments`; `startFrom` is the checkpoint's index, or 0.** §7h(b) used the
  directory's first segment, which reads more defensively and is not what upstream does — a WAL whose oldest
  segment is above the checkpoint asks for the missing one and fails. Fixed here, with the assertion that pins
  it, because the checkpoint branch is what makes `startFrom` mean anything.
- **Exception 25 is the one real divergence**: no rename in `PromFS`, so the temporary directory is copied to
  its final name. End state identical and pinned as bytes; crash window not. It is a one-method fix whenever
  something else wants `rename`.

Quirks 193 (histograms/exemplars/metadata copied through unfiltered, counted as `passedThrough`, because the
Head cannot produce them yet) and 194 (the dead `break`). Exception 25.

**The sweep found two new perl traps, and they are written into `controls-headwaltruncate.sh`'s header.** Its
first run scored 58/79 with **fifteen SKIPs**, and every one was an escaping fault rather than a corpus fault.
Nine were the new traps: inside `\Q…\E` **neither `$` nor `\` can be matched** — `\$0` does not survive
quotemeta and no number of backslashes produces a literal one, so a pattern can never contain a Swift string
interpolation. End the quoted run and use `[^\n]*` for the rest of the line, which is what
`controls-headread.sh` already does. The other six were an eight-space pattern against a twelve-space line (the
three filters sit inside a `switch` inside a `while`). `SKIP (patch did not apply)` is the only reason any of
them were noticed, which is exactly what that check is for.

#### §7i(t): the tombstone FILE codec — exception 16's other half, and the block writer's prerequisite

`tsdb/tombstones/tombstones.go`'s remaining half: `Encode`, `Decode`, `WriteFile` and `ReadTombstones`, plus
the magic/version/CRC framing and every rejection path. **94 differential cases in two arms; 52 controls
scoring 48 broke, 4 SURVIVED and all four are proofs.** A new `PromTombstonesTests` target, and
`PromTombstones` gained `PromFS`/`PromEncoding`/`PromHash`.

This is what a block carries its deletions in. `compact.go:739` writes an empty one for **every** block it
produces and `block.go:370` reads one back on `OpenBlock`, so the block writer cannot land without it — which
is why this is its own slice rather than a paragraph of one.

**The format, and the two boundaries that matter:**

```
[BE32 magic 0x0130BA30][version 0x01][ uvarint ref, varint mint, varint maxt … ][BE32 CRC-32C]
                       └───────────────── Encode() ─────────────────────────────┘
                                       └────── the CRC's input ──────┘
```

The body is a flat triple stream with no count, no index and no terminator — one triple per *interval*, so a
series with three intervals repeats its ref three times and `Decode` reads until the buffer is empty. And the
checksum starts *after* the version byte at both ends, with upstream's comment "we do this for compatibility".

**The corruption arm is where the value was, exactly as §7c predicted.** The round-trip arm (30 cases) is a
write program, so every byte it produces is well-formed and every validation in the reader is unreachable.
The second arm's input is a **file description** instead — magic, version, body, CRC, trailer and truncation
as six independently overridable parts — which is what makes "one bit flipped", "four bytes short" and "a
version byte from the future" expressible. 64 cases, and **nine of the sweep's 48 breakages are reachable from
it alone**.

**What it found: an upstream PANIC, reachable from a disk.** An eight-byte `tombstones` file crashes
Prometheus. `ReadTombstones` guards `len(b) < 5` and then slices `d.Get()[1:]` with no guard; after `Be32` has
taken the magic and the CRC has been cut off the end, `d.Get()` is `len(b) - 8` bytes, so exactly eight leaves
it empty and Go answers `runtime error: slice bounds out of range [1:0]`. `OpenBlock` has no recover. It was
found by asking what the header guard admits that nothing else rejects — five, six and seven are answered by
the magic arm, and eight is not answered by anything. Quirk 210; the port raises it as an error carrying Go's
text, per §5's `extendFloats` rule that reachability decides which treatment a panic gets.

Four things worth carrying:

- **The checksum does not cover the version byte, so a bit-flip there is reported as a version mismatch.**
  Flip `0x01` to `0x02` and the file passes its CRC and is rejected by `Decode` with `invalid tombstone
  format 2`. Presumably deliberate — a v2 file should be recognised as v2 rather than dismissed as corrupt —
  but it means the one byte that decides whether the file is parseable is the one byte the CRC skips. Quirk
  211, and the two controls that widen the hash range (one per direction) both break.
- **`Encode` walks a Go map, so a multi-series tombstone file has no byte order.** `MemTombstones.Iter`
  ranges `intvlGroups`, which means `Block.Delete` writes a *different file each time* for the same
  deletions. There is nothing to be byte-exact against, so the port sorts by ref (exception 29) and the
  oracle hands `Encode` a `tombstones.Reader` that yields the same order — the entry point stays upstream's,
  and only the axis upstream leaves undefined is fixed. Exception 11's resolution, third instance. **Whoever
  writes the block writer inherits this**: a `tombstones` file it produces is reproducible here and is not
  upstream.
- **Two error checks in the Go are dead, and the control that deletes them is an alarm rather than a gap.**
  `tombstones.go:164` and `:217` re-check the sticky `Decbuf` error after a more specific arm has already
  answered for the only read that could have latched it — which is why `Decode([])` says `invalid tombstone
  format 0` and a six-byte file says `invalid magic number 0` (quirks 212, 213). The control survives today
  and stops surviving the moment either arm above it is reordered; the companion control that *moves* the
  check breaks. That pairing is what turns "dead code" from a claim into a tested one.
- **`%x` is unpadded.** `0x000130BA` prints as `130ba`. A port reaching for `String(format: "%08x")` would be
  right on seven of the eight magic-corruption cases and wrong on the one with a leading zero byte — which is
  why that case is in the corpus and why `TombstoneCorruptTests.contractStrings` asserts it as a string.

**Exception 25 gained a second site.** `WriteFile` writes `tombstones.tmp` and `fileutil.Replace`s it; the
port copies and lets the unconditional deferred remove clean up, exactly as `checkpoint.go` does. The fixture
commits the directory LISTING as well as the file's bytes, so a surviving `.tmp` is a diff. One `rename` in
`PromFS` closes both sites.

**What is left of exception 16, and it is now a call site rather than a codec.** `BlockReader.open` still does
not call `readTombstones`, and there is no `Block.Delete`. Both belong with the block writer. The amended text
in PORTING.md says so.
...
2. **§7g — `head_read.go`** (810 lines): **LANDED**, see §7g above. `headIndexReader`, `headChunkReader`,
   `RangeHead`. It did what it promised — the corpus now asserts the samples come back rather than only that
   the counters moved — and it is what joins the Head to Phase 6's querier. What that JOIN needs is still
   open: `blockQuerier` takes an `IndexReader`/`ChunkReader` pair, and the port's two protocols were narrowed
   per call site (`PostingsIndex`, `LabelQueryIndex`), so wiring the Head into `blockQuerier` is a small
   protocol-reconciliation slice of the kind §7f(c) was.
3. **§7h — `head_wal.go`** (2,006 lines): **LANDED** in three sub-slices, see §7h(a)–(c) above. Replay, the GC,
   and the WAL truncation with its checkpoint. It was pinnable *properly* because §7b can write a WAL and §7a
   can build the records: the corpus writes a WAL, replays it, and compares the resulting Head against a Head
   built by appending the same samples. That equivalence was the whole contract, and it held.
4. **§7i — `compact.go` + `blockwriter.go`** (1,071 lines): Head to block. **This is the slice to start with.**
   **Closes §6w's two declared read-path gaps**, because it is the first thing that can put a non-XOR or
   malformed chunk in a fixture. Start with `blockwriter.go` (132 lines) — `BlockWriter` wraps a Head and a
   `LeveledCompactor` and is the smallest thing that writes a block, and its corpus can be
   `oracle/blockfixture.go` in reverse: write a block with the port, open it with `tsdb.OpenBlock`. Then
   `LeveledCompactor.Write`/`write`/`populateBlock`, deferring `CompactBlockMetas`, `plan`/`selectDirs` (that is
   `db.go`'s scheduler) and everything OOO. Then the tombstone FILE codec (`WriteFile`/`Encode`) — exception
   16's other half, needed because a block carries its tombstones. Prerequisites all landed: `index.Writer`
   (§6i), `chunks.Writer` (§6h), `meta.json` + ULID (§6k), `RangeHead` (§7g), `MemTombstones` (§7h(a)).
5. **§7j — `db.go`** (2,666 lines): the orchestration — retention, compaction scheduling, `Open`,
   `DB.Appender`/`Querier`, and `rangeForTimestamp`'s real home. Last, because everything it schedules has to
   exist first. Expect to split it: `Open` + `reloadBlocks`, then compaction scheduling, then retention.

**Scale, so it is not rediscovered:** §7i and §7j are ~3,700 lines of Go at a bar of one oracle suite plus one
argued control sweep per slice. On the evidence of the §7f–§7h sessions (nine sub-slices, each roughly 400–800
lines of Swift plus its corpus and its sweep), that is **at least two or three further sessions**, not a
remainder to be finished in one.
...
### 7i(a). `blockwriter.go` + `LeveledCompactor`'s write path — the port can produce a BLOCK

**The headline of Phase 7's remainder, and it landed byte-identical.** 36 differential cases in
`Fixtures/block/write.jsonl`, every one of them a block that upstream's own `tsdb.BlockWriter` or
`tsdb.LeveledCompactor.Write` produced, compared against the port's block as `index` bytes, chunk segment
bytes, `meta.json` bytes and the directory listing — and then read back with `tsdb.OpenBlock` on one side and
`PromBlock.Block` on the other. **Twenty-nine of the thirty-one first-run cases matched on the first
execution**; the two that did not were a real defect, and it is the interesting one (below).

New target `PromCompact`, tier 10 — a tier ABOVE `PromHead`, because a compactor reads a Head and writes a
block, and nothing beneath it may learn about compaction. 1,050 lines of Swift across four files:
`LeveledCompactor.swift`, `PopulateBlock.swift`, `BlockWriter.swift`, `BlockReaders.swift`.

#### What landed

`ExponentialBlockRanges`, `LeveledCompactorOptions`, `NewLeveledCompactor`/`NewLeveledCompactorWithOptions`,
`Write`, `write`, `DefaultBlockPopulator.PopulateBlock`, `AllSortedPostings`, the `BlockPopulator` protocol,
the whole of `blockwriter.go`, and `writeMetaFile` plus the layout constants and the two compaction hints from
`block.go`. Plus the four reader protocols and the adapters that let a `Head` or a `RangeHead` stand in for a
block.

**So the port can now do the thing it could not do before: turn samples into a block on disk that a real
Prometheus can open.**

#### The corpus, and why it has four observables

`oracle/suites_block_write.go` is `oracle/blockfixture.go` **in reverse**. That helper writes a block by hand
so the read path has an input; this one drives upstream's writer end to end and commits what came out. Four
observables, because none subsumes another:

1. the block's **file bytes** — `index`, every `chunks/NNNNNN`, `meta.json`;
2. the **directory** — what exists under `dest`, and what exists inside the block;
3. **`tsdb.OpenBlock`** over the result: labels, chunk metas, refs and samples, through real upstream code;
4. **`Flush`'s return** — the ULID or nothing, and the error string.

Two input modes, both real entry points: `blockwriter` drives `NewBlockWriter`/`Appender`/`Flush`/`Close`, and
`compactor` drives `tsdb.NewHead` plus `NewLeveledCompactor(...).Write` with an explicit range. The second
exists because `BlockWriter` cannot express a range or reach `Head.Delete` (`w.head` is unexported), and it is
the only way to reach the arms that matter most — a range that CLIPS the head, and a non-nil `base`.
`cw-blockwriter-shape-control` is the case that establishes the two modes agree.

**The ULID is scrubbed, and that is the whole trick.** `Write` names its block
`ulid.MustNew(ulid.Now(), rand.Reader)` — `crypto/rand`, reached directly, **with no seam of any kind in
v3.13.2** (the scoping note that said `NewLeveledCompactor` takes a rand source was wrong for this pin;
quirk 196). Recording the real value would make the fixture differ on every regeneration, which §4 calls worse
than no fixture. So the generator substitutes the real 26-character identifier with
`01ARZ3NDEKTSV4RRFFQ69G5FAV` in the directory name and in `meta.json`'s two ULID fields — a length-preserving
textual substitution, in the same family as the existing `scrubDir` — and the port is handed the same constant
through `LeveledCompactorOptions.newULID` (exception 27). The index and the chunk segments contain no ULID at
all and are compared untouched.

#### What the corpus caught

**One real defect, and it was not where the declaration said it would be.** §6t and §6u ported
`populateWithDelChunkSeriesIterator` with `chunkenc.NewXORChunk()` hard-coded in the re-encode path and a note
saying the other encodings were Phase 7's — a limitation declared against *deletion*. It turns out
**compacting a Head always re-encodes its last chunk, deletion or not**: the open head chunk's index meta says
`MaxTime == MaxInt64` (quirk 195), `PopulateBlock` runs with trimming enabled, so `trimBack` fires for any
range whatsoever. With `FloatChunkEncoding = EncXOR2` — which is what `promqltest` runs (quirk 36) — the port
answered `chunk N resolved to neither a chunk nor an iterable`. Two cases found it and **neither was a
deletion case**. Quirk 208, and the transferable part is the shape: *a limitation declared against one trigger
gets reached by another.*

The fix also brought `AtST()` into the re-encode loop, which is querier.go:976 and which the port had dropped
along with the XOR hard-coding.

**One gap closed that the corpus could NOT catch, and finding that out was most of an hour.**
`ChunkOrIterableWithCopy`'s third return exists to undo the index reader's `MaxInt64` (querier.go:734-737),
and §6t had not ported it. It is genuinely unobservable from a compaction — the same `trimBack` above sends
the chunk down the re-encode path, where the meta's bounds come from the surviving samples instead — and it is
reachable from the QUERIER, where `SelectHints.DisableTrimming` is what a `blockChunkQuerier` over a
`RangeHead` sets. So it is pinned in `CompactSeamTests` rather than in the corpus. Quirk 195. **Without it,
every block §7j compacts through a trimming-disabled querier would claim its last chunk ends at `MaxInt64`.**

#### The control sweep: 62 controls, 53 broke, 9 SURVIVED — every survivor a proof

`Scripts/controls-blockwrite.sh`, six source files, zero SKIP and zero COMPILE in the final run. The first run
scored 45 of 61 with three SKIPs and a COMPILE — all four harness faults, three of them the `\Q…\E`-cannot-hold-a-`$`
trap `controls-headwaltruncate.sh` already documents, and every one of them found only because the script
refuses to report a patch that did not apply. Four of the twelve survivors then became breaks once the corpus
was widened (dense samples across a clip boundary, and start timestamps) or a level was dropped
(`CompactSeamTests` for the hint sort, the version force, the `.tmp-for-creation` constant and the
no-readers guard). The nine that remain are argued in the sweep's tail; the two worth repeating here:

* **Two controls on the series-ref counter both survive, and that is quirk 197 arrived at from the other
  side.** `index.Writer.AddSeries` reads `ref` for exactly one thing — a monotonicity check — and the postings
  ordinals come from each series record's byte POSITION divided by 16 (quirk 133). So 0,1,2… and 1,2,3… and
  0,0,0… produce byte-identical files. The number is not in the file, and a corpus is not supposed to be able
  to see it.
* **The start-timestamp control survives because upstream never sets one.** `populateCurrForSingleChunk` reads
  `AtST()` per sample, and the corpus has XOR2 cases with `EnableSTStorage` and an `AppendSTZeroSample`, and
  replacing `AtST()` with `0` still passes. The reason is head_append.go:496-500 and :540 — `Append` builds
  `RefSample{Ref, T, V}` and `AppendSTZeroSample` builds `RefSample{Ref, T: st, V: 0}`, storing the start
  timestamp as a synthetic sample's *timestamp*, not as an `ST`. The only writer of a non-zero `ST` is
  `loadWAL` (head_wal.go:717). So it becomes observable the first time a compaction runs over a **replayed**
  head — a §7j corpus. Quirk 209.

#### Two API decisions taken here because §7j is the next caller

1. **`PromBlock.BlockReader` was renamed to `PromBlock.Block`, and `BlockReader` is now the protocol.** §6m's
   class was always a misnomer — its own doc comment said "Go: `tsdb.Block`" — and upstream keeps the name
   `BlockReader` for the interface that `*Block`, `*Head` and `*RangeHead` all satisfy. §7i(a) is the first
   caller of the interface (`write` takes `blocks ...BlockReader`) and §7j is the second (`NewBlockQuerier`,
   the retention loop), so the collision was settled rather than deferred. The rename touched one declaration
   and four call sites. The three sub-readers cannot take Go's names the same way — `PromChunks` already has a
   class `ChunkReader` for `chunks.Reader`, a different type from `tsdb.ChunkReader` — so they are
   `BlockIndexReader`, `BlockChunkReader` and `BlockTombstoneReader`, narrowed to what `PopulateBlock` calls,
   on §6n's precedent. Widening them for §7j is additive.
2. **The `Compactor` protocol is declared in FULL, with `plan` and `compact` throwing.** A `write`-only
   protocol would be smaller and honest about what this slice ships, and it is the wrong seam: `db.go`'s
   `compactBlocks` calls `Plan` and `Compact`, `compactHead` calls `Write`, and all three go through one
   `db.compactor` field — so a narrow protocol would have to be *widened* when §7j lands, which is
   source-breaking for every conformer and invites a second protocol beside this one. The two deferred methods
   throw `CompactError.plannerUnported` and `CompactError.verticalCompactionUnported`, both naming the slice
   that owns them, so a `db.go` built against this fails loudly at the scheduler rather than compiling into a
   TSDB that silently never compacts. `CompactSeamTests` asserts both messages.

#### What is deferred, and why each is safe

* **`Plan`, `plan`, `selectDirs`, `selectOverlappingDirs`, `splitByRange`** — `db.go`'s compaction scheduler.
  §7j. Note the §7j scoping's finding that bears on any corpus for it: `DefaultOptions()` has
  `Min == MaxBlockDuration`, which truncates `rngs` to one element, and `selectDirs` opens with
  `if len(c.ranges) < 2 … return nil` — so a corpus built on the library defaults would measure a compactor
  that is switched off. Same shape as quirk 59.
* **`Compact` and `CompactBlockMetas`** — merging N blocks needs `storage.NewMergeChunkSeriesSet`, and
  `storage/merge.go` is still unported. `PopulateBlock` therefore **refuses** more than one reader
  (`CompactError.verticalCompactionUnported`) rather than silently writing the first, and a test asserts it.
* **Everything out-of-order.** `BlockMetaCompaction.setOutOfOrder` is ported because `Write` reads the hint off
  `base`, but nothing produces an OOO block. Phase 10.
* **`CompactorMetrics` and `instrumentedChunkWriter`.** `client_golang` is not a dependency. Quirk 205 records
  the one non-obvious thing about it: it wraps the chunk writer whenever `Compaction.Level == 1`, and `Write`
  always sets level 1, so it is not the rare path its name suggests.
* **The tombstones FILE, and the remainder of exception 16.** The codec landed as §7i(t)
  (`Sources/PromTombstones/TombstoneFile.swift`); this branch predates it, so `LeveledCompactorOptions`
  carries a `TombstoneFileWriter` **seam** instead and the block has no `tombstones` file (exception 26).
  Filling the seam is a two-line follow-up. **The corpus compares `meta.json`, `index` and every chunk segment
  as BYTES and the tombstones file by NAME — there is no directory-wide `diff` to fail mysteriously.** And
  exception 29's ordering problem cannot reach this call site: `write` always passes a freshly constructed,
  *empty* `MemTombstones`, because a block that has just been populated has nothing to delete, and an empty
  map has one encoding. So the file the seam will produce is byte-comparable against Go.

  **What is left of exception 16 is NOT closed here and is explicitly out of scope**: `Block.init` still does
  not call `readTombstones`, and there is no `Block.Delete`. Both are `tsdb/block.go`'s call sites rather than
  `compact.go`'s, and §7i(t)'s codec is not on this branch. Whoever picks it up needs one `readTombstones`
  call in `PromBlock/BlockReader.swift` and a `Delete` beside it — ROADMAP calls it §7i(b).

#### §6w's two read-path gaps: ONE closed, one still open — stated plainly rather than left ambiguous

The stretch goal was to close both, on the grounds that this is the first slice that can put a non-XOR or a
malformed chunk into a fixture. Where they stand:

* **The undecodable-encoding path is now REACHED, and the port was wrong about it.** That is quirk 208 above —
  the port's `iteratorFor` and `populateCurrForSingleChunk` answered "unsupported encoding" for a perfectly
  good XOR2 chunk, and two corpus cases proved it. Both now dispatch through `newEmptyChunk`.
  ~~and the histogram encodings will fall out of it for free when `PromChunkEnc` grows them~~ — **half of that
  was right and §7k corrects the other half.** The chunk CONSTRUCTION fell out for free; the sample LOOP did
  not, because it was `guard vt == .float` and upstream's has three arms. See §7k and the amendment to quirk
  208.
* **`Err` ordering is NOT closed.** It needs a chunk reader that fails *while* a series set is being iterated,
  and neither writer in this slice can produce one: `PopulateBlock` reads through a Head whose chunks are all
  in memory or m-mapped, and a corrupted segment is `ChunkDiskMapper`'s subject (§7d), not the compactor's.
  The route in is a `Block` opened over a *deliberately truncated* chunk segment — which this slice can now
  produce, because it can write a real block to truncate — driven through `blockChunkQuerierSelect`. That is a
  `PromBlock` corpus rather than a `PromCompact` one, and it is left for whoever wants it.

#### Where to pick it up

§7j (`db.go`). Everything it schedules now exists. The two things it will want from here on day one are the
...
**The corpus.** `Fixtures/storage/merge.jsonl`, 96 cases, driven through `storage.NewMergeQuerier`
...
**Control score: 89 of 139 on the first sweep, 114 of 138 after the corpus was widened**
...
- **Two chunk shapes that a realistic case never produces.** The chunk heap orders by min time and
  breaks ties by max time; for any two chunks where neither CONTAINS the other, that order and a
  plain max-time order agree, so the control that swapped them survived until a NESTED pair was
  added. And `chunkIteratorHeap`'s tie-break itself needed two chunks starting at the same timestamp
  with different values — the corpus's blocks all had distinct starts.
...
case has more than one erroring primary. The port drops the goroutines entirely — exception 33,
...
**This slice UNBLOCKS §7i(a)'s `Compact` arm.** `DefaultBlockPopulator.populateBlock` refuses more than
one reader and throws `CompactError.verticalCompactionUnported`, with a comment naming `storage/merge.go`
as what it is waiting for. It exists now. What §7j needs to do with it is the two lines upstream's
`populateBlock` has: build a `NewMergeChunkQuerier`-shaped set over the blocks' chunk series sets and hand
`NewCompactingChunkSeriesMerger(ChainedSeriesMerge)` to it. Both are exported from `PromStorage` and both
are pinned by this slice's corpus, including the re-encoded chunk BYTES — which is the part a compaction
writes to disk, so it is the part that had to be byte-exact before this was worth unblocking. The seam is
`PromCompact` gaining a `PromStorage` dependency (it does not have one yet) and `BlockReader` growing the
chunk-series-set view; neither is decided here.
...
read. `series.go`'s `chunkSetToSeriesSet`/`seriesSetToChunkSet` likewise.

**One gap moved while this slice was in flight, and it is worth knowing which way.** The HISTOGRAM
arms of `seriesToChunkEncoder` and `chainSampleIterator`'s `consecutive` flag were scoped as blocked
on the histogram chunk encodings; §7k landed those, so both are now ported and reachable. What
remains is the FIXTURE GENERATOR: `oracle/blockfixture.go` writes float chunks only, so no merge
case can decode a histogram and two controls survive on that alone. It is a two-line change to
`blockfixture.go` and belongs with whichever slice first needs a histogram in a block.
...
### 7k. `chunkenc`'s HISTOGRAM encodings — LANDED and PINNED, and the Head's histogram path is unblocked

`tsdb/chunkenc/histogram.go` and `float_histogram.go` in full, plus the encoding half of
`histogram_meta.go` that Phase 5 left behind. **288 differential cases across two new corpora**
(`chunkenc/histogram.jsonl`, 137; `chunkenc/float-histogram.jsonl`, 151), and
`Scripts/controls-histchunk.sh` scores **96 of 108**, with five of the twelve survivors deliberately
inert and the other seven argued in the sweep's tail.

Three new files in `Sources/PromChunkEnc/`: `HistogramLayout.swift` (the layout header, `putZeroThreshold`,
`putCustomBound`, `expandSpansBothWays`, `insert`, `adjustForInserts`, `counterResetHintToHeader`),
`HistogramChunk.swift` and `FloatHistogramChunk.swift`. `newEmptyChunk` now answers for all four encodings —
the note in `ChunkConformance.swift` saying the histogram ones were absent is discharged — and `FromData`
(`chunkFromData`) and `ValueType.newChunk` landed with it.

**What this unblocks and what it does not.** `memSeries.appendHistogram`/`appendFloatHistogram` and
`headAppender`'s histogram arms are a LATER slice and deliberately out of scope here; §7f's "deferred by
feature rather than by omission" list said the absence of these two chunk types was one half of why, and
that half is now paid.

**On §6w's undecodable-encoding gap, three claims have to be separated, because two of them were loose.**

1. **The gap itself is CLOSED**, by §7i(a) for `EncXOR2` and by this slice for the two histogram encodings.
   Nothing in the read path now answers "unsupported encoding" for an encoding that exists.
2. **§7i(a)'s "the histogram encodings will fall out of it for free" was half right, and finding the other
   half is this slice's contribution to somebody else's code.** `newEmptyChunk(source.encoding)` needed
   nothing. `populateCurrForSingleChunk`'s sample loop did: it was `guard vt == .float`, upstream's has three
   arms (querier.go:977-999), and by quirk 208's own argument — that this function runs on **every**
   compaction of a Head, not only on a deletion — a histogram series would have been uncompactable with the
   port's own `populateCurrForSingleChunk: value type histogram unsupported`. Both arms are ported here, with
   upstream's `appendOnly: true` and `prev: nil`, and quirk 208 carries the amendment.
3. **`PromBlock.Block.samples` skipping histogram encodings is NOT that gap**, and an earlier draft of this
   section said it was. That method is a `(Int64, Double)` convenience of the port's own with no Go
   counterpart; a histogram sample is not a `Double` and never will be. The shape that wants them is the
   `ChunkSeriesSet` iterator, which is a read-path slice of its own and is not deferred by anything here.

The `Err`-ordering gap is untouched by this slice and remains a `PromBlock` job — see §7i(a) for the route in.

**How the two new arms are tested, since neither corpus can reach them.** `block/seriesset` and §7i(a)'s
compaction suite would both need the oracle to append a histogram through a real `tsdb.Head`, which is the
path this slice unblocks rather than the one it is; building the block bytes by hand instead is what §6w's
harness lesson forbids. So the arms are asserted at the function, in
`Tests/PromBlockTests/PopulateHistogramReencodeTests.swift`, over chunks the port's own corpus-pinned
appenders wrote — the same "drop a level" move `MatrixIterSliceTests` and `VectorElemBinopTests` made. Six
controls in `controls-histchunk.sh` perturb that file, including a restoration of the XOR hard-coding quirk
208 removed.

One thing that fell out of writing them, and it is a proof rather than a gap: **upstream's `appendOnly: true`
can never refuse here.** The shape that would refuse is a chunk holding a non-stale sample after a stale one,
because `appendable` accepts nothing after a stale sample (quirk 223) — and no such chunk exists, because the
appender that would write it cuts instead. Everything else is safe by construction: a chunk has one layout,
every sample reads back with it, and a counter chunk's counts only rise, so any in-order subset is
appendable. Both halves are asserted.

#### The two files are 90% the same and the 10% is the whole point

Read them side by side; the differences are enumerated at the top of `FloatHistogramChunk.swift` and every
one of them is a place a shared implementation would be silently wrong. The two that reach furthest:

- **Integer buckets are DELTAS and float buckets are ABSOLUTE.** `expandIntSpansAndBuckets` accumulates the
  running count where its float twin assigns, and `insert` takes a `deltas` flag whose two arms are genuinely
  different arithmetic. Quirk 225. Four controls sit on the flag's four call sites and all four break.
- **`appendable` returns a `CounterResetHeader` in the integer chunk and a `bool` in the float one**, so a
  schema change reads back as `UnknownCounterReset` from one and `NotCounterReset` from the other. Quirks
  220 and 221. **The two corpora are generated from ONE shape list** — the float suite converts every
  integer shape with `ToFloat` and then adds the float-only ones — precisely so that divergence is a
  line-for-line diff between two fixture files rather than a fact nobody notices.

#### What the corpus caught, and the one that cost the most

**The harness, again, and in a new costume.** The first run reported 83 of 130 cases mismatched and the port
was right in all 83. `AtHistogram(reuse)` returns the buffer it was handed, so the oracle's `hgcHistOut`,
which stored the returned slice *header*, made every earlier sample in a case show the LAST sample's
buckets. Swift's arrays are copy-on-write and cannot do that. **A corpus that records a Go slice from a
reused buffer is recording Go's aliasing, not Go's encoding** — snapshot at the point of observation.
`hgcHistOut` now copies and the comment there says why. Same family as §6w's three-bugs-in-three-commits;
the tell was that the *bytes* matched and only a read-back list differed.

**Four cases added after the first sweep, each closing a real gap rather than an argument:**

- `1.001` as a custom bucket bound. `1.001 * 1000` is `1000.9999999999999`, so `math.Round` encodes it in a
  four-byte varbit and a floor would spend nine bytes deciding it is not a multiple of 0.001. Every other
  bound in the first draft multiplied exactly, so the control on that rounding survived. A **harvested
  witness**, in `gocompat/log`'s sense (quirk 30) — found by searching for `k` where `Double(k)/1000 * 1000`
  falls below `k`, not by imagination.
- Two `Insert` entries at the SAME `pos`. `addInsert` merges consecutive inserted indices into one entry and
  starts a new one — with the same `pos` — when they are not contiguous, so a sample adding buckets at
  non-adjacent indices before the same old bucket produces two. That is the only input `insert`'s
  `firstInsert` flag can be seen through, and it needs the running value at that position to be NON-ZERO,
  which took a chunk covering {0, 6} against a sample covering {0, 2, 4, 6}. A second case puts both entries
  past the end of the input so they land in the trailing loop, which is where `v = 0` between entries lives.
- Non-contiguous BACKWARD inserts alongside a forward one, so `adjustForInserts` sees two entries rather than
  one merged entry. `insert` reads only `pos` and `num`, so it cannot tell them apart; `adjustForInserts`
  reads `bucketIdx` and can. The control for `addInsert`'s continuity test survived the entire first sweep
  for exactly that reason.
- `AtT()` before the first `Next()`, on a fresh iterator and on a `Reset` one. Upstream's interface calls it
  unspecified and the two constructors disagree (`math.MinInt64` versus 0, quirk 224). "Unspecified"
  describes the contract, not the bytes.

**And the corpus corrected a case that was written wrong.** `layout/backward` was `[3, 0, 0]` against a
one-bucket sample, which reads as "three buckets of 3" because integer buckets are deltas — so it produced a
counter reset where a backward insert was intended. `[3, -3, 0]` is the shape that drives the running count
to zero. The fixture said so because the counter-reset header is committed alongside the bytes; had the
corpus recorded only the samples read back, both versions would have looked fine.

#### Corpus design, since quirk 59 keeps being the lesson

`appendable` short-circuits in a fixed order — gauge, explicit hint, stale, stale, count, schema/threshold,
custom bounds, zero count, positive layout, negative layout — so a corpus whose count moves with the shape
answers at `h.Count < a.cnt` and never reaches a bucket. The `only/*` family therefore holds the count, the
sum and the layout FIXED and moves exactly one field per case, thirteen of them. Custom bounds get two
*different* sets rather than one set twice. Schemas cover both reserved ranges, because the reduction is
one-sided (quirk 229) and a port that reduced both ends would pass without the negative cases. The
`appendOnly` family exists so each of the six refusal strings is produced by the code rather than asserted
by hand; the two the corpus cannot reach — the float `layoutChange` spellings — are checked directly in
`HistogramChunkEdgeTests`.

Two directives make the corpus a *program* rather than a list: `cutBefore`, which makes the DRIVER cut the
chunk and hand the old appender over as `prev` (what `cutNewHeadChunk` does, and the only route to
`AppendHistogram`'s `prev.appendable` branch), and `reappendBefore`, which re-derives the appender from the
chunk's own bytes. The second is what exposed quirk 222: a replayed appender at one sample has `leading == 0`
where a fresh one has the `0xff` sentinel, so the second sample is encoded differently after a round trip
through `Reset`.

#### Decisions worth carrying

- **The histogram iterators are CLASSES, unlike `XORIterator`.** Go's are pointers whose `Reset` *is* the
  reuse mechanism, they carry six slices each, and they are only ever reached through `any ChunkIterator` —
  so boxing (§7f(c)'s answer for the float iterators, which are two words and have three reuse call sites in
  `PromQL`) would buy nothing and cost a layer.
- **`ChunkAppender.appendHistogram` takes `h` as `inout`.** Go's `*histogram.Histogram` is written through:
  the backward-insert path replaces `h.PositiveSpans` and `recodeHistogram` rewrites the buckets, and the
  mutation escapes — `memSeries.appendHistogram` stores the same pointer as `s.lastHistogramValue` on the
  next line. A by-value parameter would have been a divergence that only showed up in the Head. The corpus
  commits the caller's histogram after every append, so it is pinned rather than asserted.
- **Exception 31**: the iterators' `atHistogramCalled` / `atFloatHistogramCalled` slice recycling is not
  ported, because copy-on-write already gives what it buys. **Exception 32**: `Compact()` is a no-op, and the
  corpus calls it on every finished chunk and re-reads the bytes, so the one observable property is pinned.
- **Ten quirks, 220-229.** The two that will bite a later slice hardest are 220/221 (the two encodings write
  different headers for the same samples) and 223 (a stale sample leaves the encoder's and decoder's state
  disagreeing on purpose, so `Appender()` after a `Reset` is not the appender that wrote the chunk).
