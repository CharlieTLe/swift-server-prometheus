...
2. **The storage seam is narrow.** `promql/engine.go` needs only `Queryable`, `Querier`, `SeriesSet`,
   ...
   Landed at 453 lines rather than ~800, in `PromTestStorage`. The budget assumed the stand-in would
   have to be self-justifying; instead its contract is pinned against a real `tsdb.DB` in the oracle,
   which both shrank it and caught four behaviours that had been written backwards (HANDOFF §3).
...
| 5 | `PromQL` engine + storage protocols | 12k | **All 2,201 `eval` assertions green.** Shippable library milestone. **In progress** — protocol substrate, look-back iterators, `promql/value.go`, `promql/quantile.go`, the `GoMath` arithmetic layer, `promql/durations.go`, `PreprocessExpr` and the in-memory `Queryable` (`PromTestStorage`, 453 lines) done, plus a fusion audit of all 239 evaluator functions (HANDOFF §5). 4,137 annotation cases, 1,260 storage-error, 91 enum/time, 74 iterator, 245 value, 676 quantile, 40,393 GoMath, 17,665 preprocess and 40 in-memory-storage cases green. Next: the evaluator (`engine.go` + `functions.go`, 7.7k lines of Go, needs splitting), then the `.test` runner. `generic.go`/`lazy.go` and every `MarshalJSON` deferred — see HANDOFF §5 |
