...
out. What the Head still cannot do is REPLAY (`head_wal.go`, and with it `Init` and `truncateWAL`);
`compact.go` and `db.go` are untouched, so the port still cannot produce a block outside a test, and the two
...
Green as of this commit: **339,886 committed fixture lines, 653 tests** across 24 test targets, on both
...
append path **65 of 74**, the read path **51 of 63**, the GC and deletion path **41 of 52**, with every
survivor's argument written into the source or the sweep's tail, next to the code it concerns.
