...
| 5 | `PromQL` engine + storage protocols | 12k | **All 2,201 `eval` assertions green.** Shippable library milestone. **In progress** — protocol substrate, look-back iterators, `promql/value.go`, `promql/quantile.go` and the `GoMath` arithmetic layer done. 4,137 annotation cases, 1,260 storage-error, 91 enum/time, 74 iterator, 245 value, 676 quantile, 1,333 exp2, 13,335 log, 5,571 exp, 4,800 pow, 3,353 mod, 2,400 min/max, 1,918 ldexp and 7,016 `Duration.Seconds` cases green. Next: `durations.go` + `PreprocessExpr` (differentially reachable through the exported `PreprocessExpr`), then an in-memory `Queryable`, then the evaluator (`engine.go` + `functions.go`, 7.7k lines of Go, needs splitting), then the `.test` runner. `generic.go`/`lazy.go` and every `MarshalJSON` deferred — see HANDOFF §5 |
