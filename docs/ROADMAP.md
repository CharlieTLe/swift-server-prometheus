...
| 5 | `PromQL` engine + storage protocols | 12k | **All 2,201 `eval` assertions green.** Shippable library milestone. **In progress** — protocol substrate, look-back iterators, `promql/value.go`, `promql/quantile.go`, the `GoMath` arithmetic layer, `promql/durations.go` and `PreprocessExpr` done, plus a fusion audit of all 239 evaluator functions (HANDOFF §5). 4,137 annotation cases, 1,260 storage-error, 91 enum/time, 74 iterator, 245 value, 676 quantile, 40,393 GoMath and 17,665 preprocess cases green. Next: an in-memory `Queryable`, then the evaluator (`engine.go` + `functions.go`, 7.7k lines of Go, needs splitting), then the `.test` runner. `generic.go`/`lazy.go` and every `MarshalJSON` deferred — see HANDOFF §5 |
