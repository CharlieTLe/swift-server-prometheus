...
| 5 | `PromQL` engine + storage protocols | 12k | **All 2,201 `eval` assertions green.** Shippable library milestone. **In progress** — the protocol substrate, the look-back sample iterators and `promql/value.go` are done (`PromPosRange`, `PromAnnotations`, `PromChunkEnc`/`PromChunks`, `PromStorage`, `PromQL`, `GoTime`, `GoContext`, `EvalStmt`/`TestStmt`), with 4,137 annotation cases, 1,260 storage-error cases, 91 enum/time cases, 74 op-script iterator cases and 245 value cases green. Next: an in-memory `Queryable` (upstream's `promqltest` uses a real `tsdb.DB`, which Phase 5 cannot), then the evaluator, then the `.test` runner. `generic.go`/`lazy.go` and every `MarshalJSON` deferred — see HANDOFF §5 |
