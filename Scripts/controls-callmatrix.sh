...
#
# Same harness as the other two: build first, and require the "Test run with" line so a filter
# matching nothing cannot pass for green.
set -uo pipefail
cd "$(dirname "$0")/.."
...
PY2
  ...
  out=$(timeout 900 swift test --filter "EngineExec|MatrixIterSlice" 2>&1)
