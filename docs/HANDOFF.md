...
equivalence that makes a restart meaningful. What is left of Phase 7 is `truncateWAL` and the checkpoint
(§7h(c)), then `compact.go` and `db.go` — so the port still cannot produce a block outside a test, and the two
read-path gaps §6w records stay open.
...
Green as of this commit: **339,904 committed fixture lines, 659 tests** across 24 test targets, on both
...
`truncateWAL` and the checkpoint are **§7h(c)**, and the corpus has the case for them written and commented out
(`truncated-then-replayed`), because committing it now would assert the port's missing WAL truncation as a
